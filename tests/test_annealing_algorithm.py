...
from opt.algorithms.annealing import anneal, metropolis
from opt.algorithms.common import valid_initial_state, parse_constraint



def test_metropolis():
    assert True == metropolis(-2, 2, 1)


def test_anneal():
    sequence, path, distance = anneal(G, locations, start=start, end=end)
...
def test_api_with_annealing():
    sequence, path, distance = solve_tsp(G, locations, start=start, end=end, algorithm="annealing")
    assert [start] + sorted(locations) + [end] == sequence
    assert list(G) == path
    edge_weights = list(nx.get_edge_attributes(G, 'weight').values())
    assert np.isclose(
...
def test_api_with_annealing_deterministic():
    sequence, path, distance = solve_tsp(G, locations, start=start, end=end, algorithm="threshold")
    assert [start] + sorted(locations) + [end] == sequence
    assert list(G) == path
    edge_weights = list(nx.get_edge_attributes(G, 'weight').values())
    assert np.isclose(
...
def test_api_with_annealing_constrained():
    ...
    initial_state = valid_initial_state(locations, constraint)
...
def test_api_with_annealing_constrained_deterministic():
    num_priority_1_nodes = 2
    priority_1_nodes = set(np.random.choice(locations, num_priority_1_nodes, replace=False))
    not_in_priority_1 = [x for x in locations if x not in priority_1_nodes and x != start and x != end]
    num_priority_2_nodes = 5
...
def test_annealing_with_bin_data():

    G.bins = {chr(x): {"pos": (0, 0), "closest_waypoint": x-65, "distance": 0.1} for x in range(65,76)}

    test_sequence = ["A", "E", "B", "D", "G"]

    distance, sequence, path = optimize_picks(G, test_sequence, fixed_start=True, fixed_end=True, algorithm="annealing", output_df=False, cumulative_distances=False)
